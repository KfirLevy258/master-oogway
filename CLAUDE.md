...
- The `~/.<name>` → `~/.config/master-oogway/` symlinks for the four user configs — that symlink is the managed-detection mechanism (`_mo_is_managed_symlink`). Don't convert them back to plain files.
